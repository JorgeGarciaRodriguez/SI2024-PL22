...
public class H29222_Controller {
    ...
    public H29222_Controller(H29222_Model m, H29222_View v)    {
        ...
        // Acción para buscar autores
        vista.getBtnBuscarAutor().addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String criterio = vista.getTfBuscarAutor().getText().trim();
                if (criterio.isEmpty()) {
                    JOptionPane.showMessageDialog(vista.getFrame(), 
                            "Ingrese un nombre o correo para buscar", 
                            "Advertencia", 
                            JOptionPane.WARNING_MESSAGE);
                    return;
                }
                
                List<String[]> resultados = model.buscarAutores(criterio);
                ...
                for (String[] autor : resultados) {
                    listModel.addElement(autor[0] + " - " + autor[1]);
                }
                vista.getListaAutores().setModel(listModel);
            }
        });
        
        // Acción para seleccionar un autor de la lista
        vista.getListaAutores().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) {
                String seleccion = vista.getListaAutores().getSelectedValue();
                if (seleccion != null) {
                    String correo = seleccion.split(" - ")[1];
                    String[] datosAutor = model.obtenerAutorPorCorreo(correo);
                    if (datosAutor != null) {
                        vista.getTfNombre().setText(datosAutor[0]);
                        vista.getTfCorreo().setText(datosAutor[1]);
                        vista.getTfOrganizacion().setText(datosAutor[2]);
                        vista.getTfGrupInvs().setText(datosAutor[3]);
                    }
                }
            }
        });

        // Acción para añadir autor
